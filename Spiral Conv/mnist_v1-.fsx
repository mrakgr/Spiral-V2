...
let l1 = load_data (IO.Path.Combine(__SOURCE_DIRECTORY__,"weights")) false |> fun x -> ConvolutionalFeedforwardLayer.fromArray x sigmoid // sigmoid is not used here
let base_nodes = [|l1|]
...
let t' =
    l1.runLayer (defaultConvPar, x)
    |> squared_error_cost train_labels.[0]
l1.W.setZeroAdjoint() // Resets the base adjoints
l1.b.setZeroAdjoint() // Resets the base adjoints
ObjectPool.Reset() // Resets the adjoints for the training select
t'.A := 1.0f
while tape.Count > 0 do tape.Pop() |> fun x -> x() // The backpropagation step
base_nodes |> Array.iter (fun x -> saxpy -learning_rate x.W.A' x.W.P'; saxpy -learning_rate x.b.A' x.b.P') // Stochastic gradient descent.

let t = l1.runLayer (defaultConvPar, x)
let t'' = 
    t    
    |> squared_error_cost train_labels.[0]
l1.W.setZeroAdjoint() // Resets the base adjoints
